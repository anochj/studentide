...
const useDropzone = <TUploadRes, TUploadError = string>(
  ...
  const _uploadFile = useCallback(
    async (file: File, id: string, tries = 0) => {
      if (pOnDropFile === undefined) {
        throw new Error("Dropzone requires onDropFile to upload a single file");
      }
  ...
  const dropzone = rootUseDropzone({
    ...
    onDropAccepted: async (newFiles) => {
      ...
      if (pOnDropFiles !== undefined) {
        const filesWithIds = slicedNewFiles.map((file) => ({
          file,
        ...
        const result = await pOnDropFiles(slicedNewFiles);

        if (result.status === "error") {
          for (const { id } of filesWithIds) {
            dispatch({
              type: "update-status",
              id,
              status: "error",
    ...
    return (
      ...
      >
        ...
        <input
          ...
          // @ts-expect-error webkitdirectory is a non-standard input attribute used for folder selection.
          webkitdirectory={context.directoryMode ? "true" : undefined}
        />
      </label>
    );
  },
