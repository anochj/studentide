...
async function getPublicIpFromEni(networkInterfaceId: string): Promise<string> {
    ...
    if (!publicIp) {
        throw new Error("Public IP not found. The task may not have a public IP assigned.");
    }

    return publicIp;
}

async function registerToCloudflare(recordName: string, publicIp: string): Promise<void> {
    ...
    if (!data.success) {
        throw new Error(`Failed to register DNS record: ${JSON.stringify(data.errors)}`);
    }
}
...
export const handler = async (event: EventBridgeECSTaskEvent) => {
    try {
        ...
        if (!identifier) {
            console.log("No identifier found in event overrides, skipping registration.");
            return;
        }

        const eniAttachment = event.detail.attachments?.find((a) => a.type === "eni");
        const networkInterfaceId = eniAttachment?.details?.find((d) => d.name === "networkInterfaceId")?.value;

        if (!networkInterfaceId) {
            throw new Error("Network interface ID not found in the event payload attachments.");
        }
