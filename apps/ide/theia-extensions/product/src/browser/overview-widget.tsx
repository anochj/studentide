...
const ASSIGNMENT_SUBMISSIONS_URL = "https://www.studentide.com/submissions";
...
export class AssignmentOverviewWidget extends ReactWidget {
	...
	protected async loadConfig(): Promise<void> {
		...
		if (this.getDueAt()) {
			...
			this.toDispose.push(
				Disposable.create(() => window.clearInterval(countdownInterval)),
			);
		}
		this.update();
	}
	...
	protected render(): React.ReactNode {
		...
		return (
			<div className="studentide-assignment-overview-content">
				{this.renderCountdown()}
				<div className="studentide-assignment-actions">
					<button
						className="studentide-assignment-submit-button"
						type="button"
						onClick={this.openSubmissions}
					>
						Submit
					</button>
				</div>
				...
				<div
					className="studentide-assignment-overview-markdown"
					ref={this.renderMarkdown}
				/>
			</div>
		);
	}

	protected openSubmissions = (): void => {
		this.windowService.openNewWindow(ASSIGNMENT_SUBMISSIONS_URL, {
			external: true,
		});
	};
		...
		return (
			<section className={className}>
				...
				<strong>
					{isPastDue
						? "Past due"
						: this.formatRemainingTime(remainingMilliseconds)}
				</strong>
