...
/**
 * Script to build StudentIDE with a local Theia framework development version.
 *
 * This allows testing StudentIDE changes against local Theia framework modifications
 ...
 *   --skip-ide-build      Skip building StudentIDE (use for linking only) [default: false]
...
This script builds StudentIDE against a local Theia framework checkout,
...
Prerequisites:
  ...
  By default, the script expects it at ../theia (next to the StudentIDE directory).
...
function linkTheiaPackages() {
    ...
    console.log(`StudentIDE requires ${requiredDeps.size} @theia/* packages\n`);
...
function buildIde() {
    ...
    run('yarn build:applications:dev', ROOT_DIR, 'Build browser application');
 ...
 */
async function main() {
    console.log('\nBuild StudentIDE with local Theia framework\n');
    ...
    if (!options.unlink) {
        console.log(`
Next steps:
  - Run the IDE: yarn browser start
  - Make changes in Theia, rebuild: (cd ${options.theiaPath} && npm run compile)
  - Rebuild IDE: yarn build:applications:dev
